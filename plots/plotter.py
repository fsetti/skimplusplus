...
class Plotter:
    ...
    def __init__(self, **kwargs):
        ...
        if type(self.input) is str:
            ...
            self.input_options = '../samples_and_scale1fb_ul.json'
            #self.input_options = '/ceph/cms/store/user/fsetti/HiggsDNA_pickle/JetPuID_check/summary.json'
        ...
        for var, plot_info in self.plot_options.items():
            if not "signal" in plot_info.keys():
                plot_info["signal"] = ["HHggTauTau_cen"]
        if self.debug:
    ...
    def preprocess(self):
        ...
        for sample, info in self.input_options.items():
            ...
            elif "VBFH" in sample or "VH" in sample or "ttH" in sample or "ggH" in sample :
                if "VBFH" in self.process_id_map or "VH" in self.process_id_map or "ttH" in self.process_id_map or "ggH" in self.process_id_map :
                    continue
                else:
                    self.process_id_map["Single H"] = info["process_id"]
                    self.master_dataframe["Single H"] = self.input[
                        self.input["process_id"] == info["process_id"]
                    ]

            elif "TT" in sample:
                if "TT" in self.process_id_map:
                    continue
                else:
                    self.process_id_map["tt+X"] = info["process_id"]
                    self.master_dataframe["tt+X"] = self.input[
                        self.input["process_id"] == info["process_id"]
                    ]

            elif "WGamma" in sample or "ZGamma" in sample:
                if "WGamma" in self.process_id_map:
                    continue
                if "ZGamma" in self.process_id_map:
                    continue
                else:
                    self.process_id_map["VGamma"] = info["process_id"]
                    self.master_dataframe["VGamma"] = self.input[
                        self.input["process_id"] == info["process_id"]
                    ]
    ...
    def make_plots(self):
        ...
        for idx, branch in enumerate(self.branches):
            ...
            nonSM_couplings = ["HHggTauTau_cen_EFT_node_1", "HHggTauTau_cen_EFT_node_2", "HHggTauTau_cen_EFT_node_3", "HHggTauTau_cen_EFT_node_4", "HHggTauTau_cen_EFT_node_5", "HHggTauTau_cen_EFT_node_6", "HHggTauTau_cen_EFT_node_7", "HHggTauTau_cen_EFT_node_8", "HHggTauTau_cen_EFT_node_9", "HHggTauTau_cen_EFT_node_10", "HHggTauTau_cen_EFT_node_11", "HHggTauTau_cen_EFT_node_12", "NMSSM_XYH_Y_tautau_H_gg_MX_300_MY_125", "NMSSM_XYH_Y_tautau_H_gg_MX_500_MY_300", "NMSSM_XYH_Y_tautau_H_gg_MX_700_MY_500" , "NMSSM_XYH_Y_tautau_H_gg_MX_1000_MY_700" ]
            for process, hist in self.histograms[branch].items():
                if process == "signal" or process == "Data" or process in nonSM_couplings or "ggf" in process or "vbf" in process or process in res_bkg or "NMSSM" in process:
                    continue
                else:
            ...
            if "signal" in self.histograms[branch]:
                ...
                self.histograms[branch]["signal"].plot(
                    histtype="step", label=signal_label, ax=ax1, color="black", linewidth=3
                )
            ...
            for proc in nonSM_couplings:
              if proc in self.histograms[branch]:
                  if "HHggTauTau_cen" in proc:
                  	proc_label = proc.split("HHggTauTau_cen_")[-1].replace("_node","")
                  else:
                  	proc_label = proc
                  if unit_normalize:
                      self.histograms[branch][proc] /= self.histograms[
                  ...
                  self.histograms[branch][proc].plot(
                      histtype="step", label=proc_label, ax=ax1, linewidth=3, #color="black"
                  )
            ...
            ratio_hist = self.histograms[branch]["signal"].copy()
            ...
            ratio_hist.plot(ax=ax2, show_errors=True, label="prv/cen")
            ax1.legend(fontsize=18)
            ''' 
...
if __name__ == "__main__":
    p = Plotter(
        #df="/ceph/cms/store/user/fsetti/HiggsDNA_pickle/JetPuID_check/jet1.pkl",
        df="/home/users/fsetti/HHggTauTau/HggAnalysisDev/MVAs/out_zipped/run2_POGsel_04May2022.pkl",
