...
class Plotter:
    ...
    def __init__(self, **kwargs):
        ...
        if type(self.input) is str:
            ...
            self.input_options = 'xyh_summary.json'
        ...
        for var, plot_info in self.plot_options.items():
            if not "signal" in plot_info.keys():
                plot_info["signal"] = ["NMSSM_XYH_Y_gg_H_bb_MX_650_MY_95"]
        if self.debug:
    ...
    def preprocess(self):
        ...
        for sample, info in self.input_options.items():
            ...
            if "GJets" in sample:
                ...
                if "M125" in self.process_id_map:
                    continue
                else:
    ...
    def make_plots(self):
        ...
        for idx, branch in enumerate(self.branches):
            ...
            hist_stack = []
            res_bkg = [ "VH", "ttH", "VBFH", "ggH", "Single H" ]
            nonSM_couplings = []
            ...
            for proc in nonSM_couplings:
              if proc in self.histograms[branch]:
                  if "NMSSM_XYH_Y_gg_H_bb_MX_650_MY_95" in proc:
                  	proc_label = 'XYH'
                  else:
                  	proc_label = proc
                  if unit_normalize:
...
if __name__ == "__main__":
    #p = Plotter(
    #    df="/ceph/cms/store/user/fsetti/XYH_NMSSM_BDT/out_zipped/xyh_nmssm_nominal_21Sep2022.pkl",
